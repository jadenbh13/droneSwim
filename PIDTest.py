...
def wait_for_position_estimator(scf):
	...
	with SyncLogger(scf, log_config) as logger:
		for log_entry in logger:
			...
			pv = var_x_history.pop(0)
			var_y_history.append(data['kalman.varPY'])
			py = var_y_history.pop(0)
			var_z_history.append(data['kalman.varPZ'])
			pz = var_z_history.pop(0)
			...
			print(f"{data['kalman.varPX']}, {data['kalman.varPY']}, {data['kalman.varPZ']}")
...
def setPose(cf, xCord, yCord):
	...
	cf.commander.send_position_setpoint(xCord, yCord, 0.5, 0)
...
# sins returns Absolute Positioning
def sins(real, im, posX=0, posY=0):
...
def run_sequence(cf, sequence):
	...
	try:
		...
		# Set Base Positions of Drone for Updating:
		BasePosX,BasePosY = 0,0
		while True:
			try:
				end = time.time()
				el = end - start
				...
				# Send in posX and posY

				si = sins(amX, amY, BasePosX, BasePosY)

				# mX & mY hold Absolute Positioning
				mX = si[0]
				mY = si[1]
				tm = time.time()
				...
				BasePosX,BasePosY = si[0], si[1]

				fileSet("PID_Test_1.txt", mX, mY, el, tm)
